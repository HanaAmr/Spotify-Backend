...
describe('userController assigning cancellation code to user functionality', () => {
  ...
  // Testing assigning the cancellation code to a non existent user
  ...
  // Testing assigning the cancellation code to an already normal user
  it('Shouldn\'t assign the cancellation code as it\'s already a normal user', async (done) => {
    // get the id of the document in the db to use it to get authorization token
    await User.findOne({}, async (err, user) => {
      const id = user._id
      authToken = jwt.sign({ id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE_IN })
  ...
  // Testing assigning the cancellation code with no authorization token
  it('Shouldn\'t assign the cancellation code as no authorization token is passed', async (done) => {
    const request = httpMocks.createRequest({
      method: 'DELETE',
      url: '/me/premium',
      headers: {}
