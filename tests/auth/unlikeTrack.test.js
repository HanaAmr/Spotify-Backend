...
describe('Unlike track functionality', () => {
  let authToken = ''
  let id = ''
  let trackId = ''
  let trackId2 = ''
  ...
  beforeEach(async () => {
    ...
    // Creating a user to unlike track
    const user = new User({
      name: 'ahmed',
  ...
  // Testing unlike track successfully
  it('Should unlike track successfully', async () => {
    const response = await supertest(app).delete('/me/unlikeTrack').send({
      id: trackId
  ...
  it('Should not unlike track', done => {
  ...
  it('Should not unlike track', done => {
    ...
    const response = httpMocks.createResponse()
    authContoller.unlikeTrack(request, response, (err) => {
      try {
        expect(err).toEqual(expect.anything())
        expect(err.statusCode).toEqual(400)
        expect(err.status).toEqual('fail')
